...
class Listener {
    ...
    verifyClient(info, response) {
        console.log(info.req.headers['fly-client-ip'],111)
        const address = filterIPAddress(info.req.headers['fly-client-ip'] || info.req.socket.remoteAddress);
    ...
    onConnection(webSocket,request) {
        ...
        //console.log(newConnection.remoteAddress,this.connectionsByIP[newConnection.remoteAddress])
    }
